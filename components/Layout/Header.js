    ...
    <header className="w-full">
      <div className="bg-[#003C22] text-white">
        <div className="container mx-auto flex justify-between items-center px-4">
          ...
          <button className="hidden md:flex bg-[#FD820B] text-white px-3 md:px-4 py-1.5 md:py-2 text-xs md:text-sm font-medium items-center gap-2 hover:bg-orange-700 shadow rounded">
      ...
      <div className="bg-white border-b border-[#E0E2E7]">
        <div className="flex justify-between items-center container mx-auto px-4 py-4 md:py-5">
          ...
          <div className="flex-1 mx-4 md:mx-8 relative hidden sm:block">
            <button className="absolute left-1 top-0 bottom-0 m-auto w-[40px] h-[40px] flex justify-center items-center rounded-full">
              <SearchIcon className="text-gray-500" />
            </button>
            <input
              type="text"
              placeholder="Search your product, category or brands"
              className="w-full lg:w-[470px] min-h-[42px] pl-[48px] pr-4 py-2 bg-[#F3F3F5] border border-[#E0E2E7] text-[#4D4D4D] placeholder-[#4D4D4D] rounded-full text-[14px] font-medium focus:outline-none focus:ring-2 focus:ring-green-500"
            />
          </div>
          ...
          <div className="flex items-center gap-4 text-[#003C22]">
            ...
            <div className="hidden md:block relative">
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value)}
                className="border text-[14px] md:text-[15px] font-semibold px-3 md:px-4 md:pr-8 rounded-[10px] pe-8 py-1.5 md:py-2 appearance-none border-[#003C22]"
              >
                <option>English</option>
                <option>Hindi</option>
              </select>
              <FaChevronDown className="absolute top-0 bottom-0 right-2 m-auto text-xs" />
            </div>
      ...
      >
        <ul className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-center px-6 py-3 gap-4 sm:gap-8 text-gray-700 font-medium text-sm">
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
            Brands <FaChevronDown size={12} />
          </li>
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
            Fertilizers <FaChevronDown size={12} />
          </li>
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
            Seeds <FaChevronDown size={12} />
          </li>
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
            Fungicides <FaChevronDown size={12} />
          </li>
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
            Herbicides <FaChevronDown size={12} />
          </li>
          <li className="text-[15px] font-medium tracking-wider flex items-center gap-2 text-[#003C22] cursor-pointer hover:text-green-700">
